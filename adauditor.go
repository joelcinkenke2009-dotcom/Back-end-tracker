...
func (e *Env) HandleMetaCallback(w http.ResponseWriter, r *http.Request) {
	...
	query := `
		INSERT INTO user_meta_ad_connect (user_id, access_token, expires_at) VALUES (?, ?, ?)`
...
log.Println(user)
	...
	err := e.db.QueryRow(`SELECT access_token FROM user_meta_ad_connect WHERE user_id=?`, user).Scan(&accessToken)
	if err != nil {
